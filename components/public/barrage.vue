...
export default {
  ...
  data () {
    return {
      ...
      CHANNEL_COUNT: 8, // 弹幕数组

      barrages: [], // dom池

      domPool: [], // intervalDM

      intervalDM: null, // 取弹幕时间间隔

      interValTime: 500, // 滚动弹幕的通道

      hasPosition: [], // 顶部弹幕的通道

      hasTopPosition: [], // 弹幕容器

      barrageMainDm: null, // 弹幕容器宽度

      barMainWidth: 500, // 自定义弹幕样式属性列表
  ...
  mounted () {
    this.barrageMainDm = this.$refs.barrageMainDm; // 缓存容器宽度

    this.barMainWidth = this.barrageMainDm.clientWidth; // 初始化弹幕dom组
  ...
  methods: {
    ...
    init () {
      let wrapper = this.$refs.barrageMainDm; // 先new一些div 重复利用这些DOM
      ...
      for (let j = 0; j < this.CHANNEL_COUNT; j++) {
        ...
        for (let i = 0; i < this.MAX_DM_COUNT; i++) {
          ...
          let dom = document.createElement("div"); // 初始化dom的位置 通过设置className

          dom.className = "barrage-item";

          dom.style.transform = `translate3d(${this.barMainWidth}px,0,0)`; // DOM的通道是固定的 所以设置好top就不需要再改变了

          dom.style.top =
            j * (this.barrageMainDm.clientHeight / this.CHANNEL_COUNT) + "px";

          console.log(dom.style.top); // 每次到animationend结束的时候 就是弹幕划出屏幕了 将DOM位置重置 再放回DOM池
          ...
          dom.addEventListener("animationend", (e) => {
            ...
            dom.style.animation = null; // 清空自定义样式

            this.dmStyles.forEach((key) => {
              dom.style[key] = null;
            }); // this.domPool[j].push({ el: dom }); // 动画结束后设置当前dom为空闲状态 // console.log(i, j)
...
    */,
    ...
    shootDanmu (domItem, dmItem, channel) {
      ...
      this.hasPosition[channel] = false; //获取dom

      let dom = domItem.el; // 是否已经使用的dom
      ...
      domItem.isFree = true; // console.log('biu~ [' + dmItem.content + ']'); // dom.innerText = dmItem.content; // 判断是否是js弹幕

      if (dmItem.isJs) {
        dom.innerHTML = dmItem.content;
      } else {
        dom.innerText = dmItem.content;
      } // 设置弹幕的位置信息 性能优化 left -> transform // dom.style.transform = `translateX(${-dom.clientWidth}px)`; // dom.style.transform = `translate3d(${-dom.clientWidth}px,0,0)`; // css硬件加速

      dom.className = dmItem.isSelf ? "barrage-item self-dm" : "barrage-item";

      dom.style.animation = "barrage-run 5s linear"; // 弹幕全部显示之后 才能开始下一条弹幕 // 大概 dom.clientWidth * 10 的时间 该条弹幕就从右边全部划出到可见区域 再加1秒保证弹幕之间距离
    ...
    shootTopDanmu (dmItem, channel) {
      ...
      this.hasTopPosition[channel] = false; //获取dom

      let dom = document.createElement("div");

      dom.className = dmItem.isSelf
        ? "barrage-item top-item self-dm"
        : "barrage-item top-item"; // 判断是否有自定义样式
      ...
      dom.style.transform = `translate3d(${this.barMainWidth / 2 - dom.clientWidth / 2
